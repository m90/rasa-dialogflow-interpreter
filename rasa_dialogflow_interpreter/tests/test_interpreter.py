...
from rasa_dialogflow_interpreter.interpreter import (
    build_response,
    build_entity,
...
class TestBuildEntity(unittest.TestCase):
    def test_default(self):
        expected = {"entity": "things", "value": "many", "start": 0, "end": 0}
...
class TestBuildResponse(unittest.TestCase):
    ...
    def test_no_entities(self):
        expected = {
            ...
            "intent": {"name": "be-lazy", "confidence": 1.0},
            "entities": [],
        }
        result = build_response(
            "no entities needed", intent={"name": "be-lazy", "confidence": 1.0}
        )
        self.assertDictEqual(expected, result)
